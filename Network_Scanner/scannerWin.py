...
def pingSweeping(ip_address, minip, maxip):
        ...
        for response in output.readlines(): # Responses differ from OS to OS
            if(operatingSystem == "Linux"):
                if("bytes from" in response):
                    print("\t[+] Host with IP address {} is ALIVE".format(tested_ip_address))
                    break
                elif("Destination Host Unreachable" in response):
                    print("\t[-] Host with IP address {} may be DEAD or firewall is ON".format(tested_ip_address))
                    break
    ...
    print("[*] Ping sweeping finished in: {}".format(totalTime))

def tcp_full_open_scan(target, minPort, maxPort):
    if(target == None):
        print('\n' + "[!] You have to specify the address")
    else:
        ...
        try:
            ...
            finishTime = datetime.now()
            totalTime = finishTime - startTime
            print("[*] Scanning finished in: {}".format(totalTime))
        except:
            print('\n' + "An Exception has been risen")

def main():
    mode, address, minimum_host_ip, maximum_host_ip, minimum_port_number, maximum_port_number = arguments()
    signature()
    if(mode is None):
        print('\n' + "[!] You have to specify the mode")
    else:
        if((mode == 'icmp') and (address is not None)):
            ...
            print('\n' + "[!] There is not such mode. Try again")
            os.system(exit(1))
